...
import GrowthAddModal from '@/components/modals/GrowthAddModal'
...
export default function GrowthManagement() {
  ...
  const [formData, setFormData] = useState<Partial<Growth>>({})
  ...
  const fetchGrowthData = async () => {
    try {
      ...
      const response = await fetch(`/api/growths?${params}`);
      ...
      setError('성장 데이터 조회 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/growths?id=${id}`, {
        method: 'DELETE'
      });

      if (!response.ok) throw new Error('삭제에 실패했습니다.');

      setAlertMessage('성장 데이터가 삭제되었습니다.');
  ...
  // 폼 제출 처리
  const handleSubmit = async (formData: Partial<Growth>) => {
    try {
      const url = editingId ? `/api/growths?id=${editingId}` : '/api/growths';
      ...
      setAlertMessage(editingId ? '성장 데이터가 수정되었습니다.' : '성장 데이터가 등록되었습니다.');
      setAlertType('success');
      handleModalClose();
      fetchGrowthData();
    } catch (error) {
          ...
          <table className="w-full text-xs text-left text-gray-500 dark:text-gray-400 border-collapse border border-gray-300">
            ...
            <tbody className="text-[11px]">
              {growthData.map((growth) => (
                <tr key={growth.id} className="bg-white dark:bg-gray-800">
                  <td className="py-2 px-3 text-center border border-gray-300">{growth.student.name}</td>
                  <td className="py-2 px-2 text-center border border-gray-300">{growth.student.grade}</td>
                  <td className="py-2 px-2 text-center border border-gray-300">{growth.student.classNumber}</td>
                  <td className="py-2 px-2 text-center border border-gray-300">{growth.student.studentNumber}</td>
                  <td className="py-2 px-2 text-center border border-gray-300">
                    {growth.student.gender === 'male' ? '남' : '여'}
                  </td>
                  <td className="py-2 px-3 text-center border border-gray-300">{growth.height}</td>
