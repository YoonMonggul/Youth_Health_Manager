...
interface ProgramEnd {
  ...
  endedAt: string;
  reason: string;
  students: { id: number; name: string; grade: number; classNumber: number; studentNumber: number; gender: 'male' | 'female'; schoolType: string }[];
}
...
export default function ProgramDetail({ programEndId }: ProgramDetailProps) {
  const [programEnd, setProgramEnd] = useState<ProgramEnd | null>(null);
  ...
  // 종료된 프로그램 상세 정보 불러오기
  const fetchProgramEnd = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/programs/program-ends/${programEndId}`);
      if (!res.ok) throw new Error('프로그램 상세 정보 조회 실패');
  ...
  // 프로그램 정보가 로드되면 관련 데이터도 불러오기
  useEffect(() => {
    if (programEnd) {
      // 종료된 프로그램의 경우 원본 programStartId가 필요할 수 있음
      // 여기서는 임시로 programEnd.id를 사용
      fetchProgramLogs(programEnd.id);
      // 주차별 평균값 데이터도 불러오기
      fetchWeeklyAverageData(programEnd.id);
    }
  }, [programEnd, fetchProgramLogs, fetchWeeklyAverageData]);
  ...
  const selectedStudents: Student[] = useMemo(() => {
    return (programEnd?.students as Student[]) || [];
  }, [programEnd?.students]);
    ...
    return (
      <div className="text-center py-12">
        <div className="text-gray-500 mb-4">
          <div className="text-lg font-medium">프로그램을 찾을 수 없습니다</div>
          <div className="text-sm mt-1">프로그램이 삭제되었거나 존재하지 않습니다.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#F7F8FA]">
      ...
      <div className="flex justify-between items-center mb-6 px-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{programEnd.programName}</h1>
          <p className="text-gray-600 mt-1">
            {new Date(programEnd.startDate).toLocaleDateString('ko-KR')} ~ {new Date(programEnd.endDate).toLocaleDateString('ko-KR')} 
            (종료됨 - {new Date(programEnd.endedAt).toLocaleDateString('ko-KR')})
          </p>
        </div>
        ...
        <div className="col-span-4 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-lg shadow p-4 h-full flex flex-col justify-between">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-lg font-bold">{getTrendTypeLabel(weeklyAverageData?.trendType)}</h3>
              <span className="text-xs text-gray-500">
                {weeklyAverageData ? `${weeklyAverageData.totalWeeks}주간 평균 ${getTrendTypeLabel(weeklyAverageData.trendType).replace('변화율 추이', '')}` : '데이터 로딩 중...'}
              </span>
            </div>
            {/* 실제 데이터 기반 그래프 */}
            {loadingWeeklyData ? (
      ...
      <ParticipantDetailModal
        ...
        programName={programEnd.programName}
      />
    </div>
  );
} 